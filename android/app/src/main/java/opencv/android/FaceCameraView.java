...
public class FaceCameraView extends SimpleViewManager<FrameLayout> implements LifecycleEventListener {
    ...
    protected FrameLayout createViewInstance(final ThemedReactContext reactContext) {
        ...
        camera.setFaceCapturedCallBack(new RecognitionMethods.onFaceCaptured() {
            @Override
            public void onFaceBack(String faceBitmap) {
                ReactContext context = reactContext;
                WritableMap event = Arguments.createMap();
                event.putString("faceBitmap", faceBitmap);
                context.getJSModule(RCTEventEmitter.class).receiveEvent(preview.getId(),"faceCapturedCompleted", event);
            }
        });
